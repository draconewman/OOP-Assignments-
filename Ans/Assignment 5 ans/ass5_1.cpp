/*1.	There are number of students. For every student roll (unique), name is to be stored. For each subject, subject code and name is to be stored. 
A student can opt for number of subjects. System should be able to maintain student list, 
subject list and will be able to answer: i) which student has selected which subjects and ii) for a subjects who are the students.
...
int student=0;      //declaring global variables
int cseStudent=0;
	...
	void admission(int c,int y)
	{
		...
		switch(deptChoice)
	  {
	  	case 0:
	  		...
	  		cseStudent++;
	  		roll=y*1+code*1+(count[code]+cseStudent);
	  		cout<<"Admission is successful..\n\tStudent roll number is -- "<<roll<<"\n";
			break;
	  	case 1:
	  		
	  		electroStudent++;
	  		roll=y*1+code*1+(count[code]+electroStudent);
	  		cout<<"Admission is successful..\n\tStudent roll number is -- "<<roll<<"\n";
			break;
	  	case 2:
	  		
	  		electricStudent++;
	  		roll=y*1+code*1+(count[code]+electricStudent);
	  		cout<<"Admission is successful..\n\tStudent roll number is -- "<<roll<<"\n";
			break;
	  	case 3:
	  		
	  		mechStudent++;
	  		roll=y*1+code*1+(count[code]+mechStudent);
	  		cout<<"Admission is successful..\n\tStudent roll number is -- "<<roll<<"\n";
			break;
	  	default :
...
{
	//validating date
	if(y<1900 || y>2022 || m<1 || m>12 || d<0 || d>31){
		cout<<"Wrong date\nEnter once again\n";
		return 0;
	}
...
int main()
{
	...
	cout<<"WELCOME TO JADAVPUR UNIVERSITY\n";
	do
	{
		cout<<"\n\nEnter the operation:\n"
		    <<"1.Admission\n"
